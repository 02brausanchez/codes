    ...
    */
    ...
    private Nodo insertaOrdenado2(Nodo a, char from, Nodo ant, char info) {
        if (a == null) {
            ...
            actualizarLigas(ant, from, nuevo);
            return nuevo;
        }
    ...
    private void actualizarLigas(Nodo ant, char from, Nodo nuevo) {
        if (ant == null) { // primer nodo del árbol
            l = nuevo;
        } else if (from == 'i') { // viene por la izquierda
            nuevo.suc = ant;
            nuevo.prev = ant.prev;
            ant.prev = nuevo;
            if (nuevo.prev != null) {
                nuevo.prev.suc = nuevo;
            } else {
                l = nuevo;
            }
        } else { // viene por la derecha
            nuevo.prev = ant;
            nuevo.suc = ant.suc;
            ant.suc = nuevo;
            if (nuevo.suc != null) {
                nuevo.suc.prev = nuevo;
            }
        }
    }
    public void insertaOrdenado2(char info) {
